...
# controller_v4_full_sensor_bypass_20260717 exclusionary pilot: single-robot,
# long-duration static-neighbor wooden-box local avoidance. Same world,
# initial pose, and base parameters as pilot_a2/pilot_a3 (identical scenario,
# only the controller code differs). No local_v4_* parameter is overridden
...
rclpy.init()
node = rclpy.create_node(f"static_v4_{stage.lower()}_clock_rate_check")
samples = []
...
echo "[$(date -Iseconds)] $STEM START (peer_avoidance=$PEER_AVOIDANCE)" | tee "$EXECUTION_LOG"
...
stdbuf -oL -eL ros2 run epuck2_comm cooperative_avoider --ros-args \
  ...
  -p enable_peer_avoidance:="$PEER_AVOIDANCE" \
