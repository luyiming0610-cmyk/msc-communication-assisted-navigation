#!/usr/bin/env python3
#
# pilot_v4_combined: same geometry, startup holds, and CPA/local parameters
# as the pre-v4 pilot_04 combined_wood_moving_peer configuration (the one
# that already proved the CPA sync was correct, before v1's runaway-turn
# defect was found near the box's trailing corner). Only the underlying
# cooperative_avoider/local_obstacle_logic package is v4; the task monitor
# is combined_task_coordinator_v4.py (one pre-diagnosed encounter-distance
# sampling fix only, see that file's docstring).
...
def main():
    ...
    coordinator = ExecuteProcess(
        cmd=[sys.executable, str(CONFIG_DIR / "combined_task_coordinator_v4.py")],
        output="screen",
    )
