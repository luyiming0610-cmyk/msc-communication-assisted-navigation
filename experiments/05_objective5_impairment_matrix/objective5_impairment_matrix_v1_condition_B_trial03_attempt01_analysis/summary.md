# FORMAL_SIM -- Condition B, Trial 03, attempt 01

`objective5_impairment_matrix_v1_condition_B_trial03_attempt01` -- part of a formal n=5 batch for Condition B.
...
|---|---|---|
| sample_count | 438 | 446 |
| missing/duplicate/out_of_order | 0/0/0 | 0/0/0 |
| capture_ratio | 1.0 | 1.0 |
| mean/median/p95/p99/max age (s) | 0.2086301369885844/0.20000000000000284/0.21999999999999886/0.21999999999999886/0.21999999999999886 | 0.2090134529170403/0.20000000000000284/0.21999999999999886/0.21999999999999886/0.21999999999999886 |
| latency_measurement_status | VALID_AT_SIM_CLOCK_RESOLUTION | VALID_AT_SIM_CLOCK_RESOLUTION |
| mean bandwidth (bytes/s) | 706.6338119029846 | 706.833989024102 |
...
- `ros2 bag info` succeeded (3224 messages, 7 topics, 54.432986689s duration).
- `preload_realtime_factor=0.963`, `full_load_realtime_factor=0.96`.

## Safety

`min_interrobot_distance_m=0.1464050217802145`, `safety_radius_m=0.14`, `safety_margin_m=0.006405021780214487`.

## Process cleanup / data integrity

- Post-run process check: **CLEAN**.
- Native WSL bag copied to `experiments/05_objective5_impairment_matrix/bags/objective5_impairment_matrix_v1_condition_B_trial03_attempt01/` (gitignored) -- SHA-256 verified identical to native WSL source (bag + diag_logs). See `SHA256SUMS`.
