...
| `/epuck_bridge/status`               | (bridge-specific, existing) | existing WSL<->Pi bridge (unmodified) | recorder, read-only connectivity check | Reused from `06_physical_pipuck` bridge tooling. Corrected 2026-07-29: this row previously read `/hil/bridge_status`, which never matched the real topic -- `hil_command_evidence_recorder.py`'s actual `--bridge-status-topic` default and `HIL_LAB_RUNBOOK.md`'s own usage (`ros2 topic echo /epuck_bridge/status`) both use `/epuck_bridge/status`. No code changed; only this documentation row was stale. |
