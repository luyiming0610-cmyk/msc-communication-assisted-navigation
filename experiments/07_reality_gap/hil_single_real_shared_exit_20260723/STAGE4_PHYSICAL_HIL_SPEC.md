...
The finalizer does not require a separate Pi verdict generated by the
older zero-only ground-diagnostic verifier: expected Stage 4 motion is
incompatible with that diagnostic's zero-command acceptance rule.
Instead, the Stage 4 verifier parses the transferred Pi command-audit
JSONL and WSL command-evidence CSV directly.
...
terminal-state/disarm sequence (while permitting only safe
`RAW_TWIST_IGNORED` records after `LATCHED_COMPLETE`),
hash/PID-manifest/residual-process evidence when paths are supplied,
and (physical mode only) explicit operator measurements. Physical mode
also directly verifies both command streams contain bounded nonzero
linear motion, zero angular motion, an ending zero command, and valid
bridge/Pi shutdown evidence. Missing or unparseable evidence is always
`INVALID_EVIDENCE`, never defaulted to zero/false/success.
