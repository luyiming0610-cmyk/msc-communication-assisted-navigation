...
# controller_v3_unified_encounter_20260717 exclusionary pilot (pilot_a3):
# single-robot, long-duration static-neighbor wooden-box local avoidance.
# Reuses the existing, unmodified controller_v1/v2 simulation infrastructure
...
# controller_v3_unified_encounter code. Same world, same initial pose, same
# max_runtime_s=55.0/stop_after_recovery=false base parameters as pilot_a2
# (controller_v2_local_latch_20260717_static_box_pilot_a2) -- identical
# scenario, only the controller code differs, so any behaviour change is
# attributable to controller_v3 and nothing else. No local_encounter_*
# parameter is overridden here: the node's own shipped defaults
# (max_turn_ledger_rad=0.70, max_bypass_extension_m=0.30, etc.) apply,
# matching pilot_a2's use of controller_v2's shipped defaults.
#
# This pilot is EXCLUDED from all formal statistics. Its pass/fail verdict
# is NOT this script's exit code or its final echo line -- "COMPLETE:
# maximum runtime reached" is emitted by the controller whether or not the
# encounter actually resolved safely (see cooperative_avoider.py _control()),
# so reaching the watchdog's COMPLETE line only means "the run finished and
# produced a bag", not "pilot_a3 passed". The real verdict is decided
# afterwards, by hand, against the pilot_a3 acceptance checklist, using this
# script's own analyzer outputs (combined_task_summary.json and
# static_v3_controller_log_summary.json).
...
rclpy.init()
node = rclpy.create_node(f"static_v3_{stage.lower()}_clock_rate_check")
samples = []
...
if (( recording_complete == 0 )); then
  ...
  echo "[$(date -Iseconds)] controller reached its COMPLETE line (this only means the run finished, not that pilot_a3 passed -- see acceptance checklist)" | tee -a "$EXECUTION_LOG"
fi
...
python3 "$CONFIG_DIR/analyze_static_v3_controller_log.py" "$BAG_DIR" "$CONTROLLER_LOG" >>"$EXECUTION_LOG" 2>&1
