...
CONFIG_DIR="$EXPERIMENT_DIR/config/ablation_head_on_fused"
STEM="ablation_fused_head_on_realtime_formal_trial_${TRIAL}"
...
rclpy.init()
node = rclpy.create_node(f"fused_{stage.lower()}_clock_rate_check")
samples = []
...
stdbuf -oL -eL python3 "$CONFIG_DIR/run_fused_head_on_controllers.py" \
  >"$CONTROLLER_LOG" 2>&1 &
CONTROLLER_PID=$!
...
if (( complete_count < 2 )); then
  echo "Timed out waiting for both cooperative COMPLETE messages" >&2
  exit 1
fi
...
local_mode_count="$(grep -c 'mode=LOCAL_' "$CONTROLLER_LOG" 2>/dev/null || true)"
...
echo "LOCAL_MODE_LOG_COUNT=$local_mode_count" | tee -a "$EXECUTION_LOG"
...
print(
    ...
    "control_input=fused_local_and_peer"
)
PY
