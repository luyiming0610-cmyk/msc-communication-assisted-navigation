...
CONFIG_DIR="$EXPERIMENT_DIR/config/ablation_head_on_local_only"
STEM="ablation_local_only_head_on_realtime_formal_trial_${TRIAL}"
...
rclpy.init()
node = rclpy.create_node(f"local_only_{stage.lower()}_clock_rate_check")
samples = []
...
stdbuf -oL -eL python3 "$CONFIG_DIR/run_local_only_head_on_controllers.py" \
  >"$CONTROLLER_LOG" 2>&1 &
CONTROLLER_PID=$!
...
complete_count=0
while (( SECONDS < deadline )); do
  complete_count="$(grep -c 'COMPLETE: local recovery completed; commanding zero' "$CONTROLLER_LOG" 2>/dev/null || true)"
...
disabled_count="$(grep -c 'peer-state CPA avoidance disabled' "$CONTROLLER_LOG" 2>/dev/null || true)"
local_warn_count="$(grep -c 'mode=LOCAL_FRONT_WARN' "$CONTROLLER_LOG" 2>/dev/null || true)"
if (( disabled_count < 2 )); then
  echo "Peer-avoidance disable confirmation missing" >&2
  exit 1
fi
if (( local_warn_count < 2 )); then
  echo "Both robots did not confirm local front-triggered avoidance" >&2
  exit 1
fi
if grep -q 'COMPLETE: maximum runtime reached' "$CONTROLLER_LOG"; then
  echo "Run ended by safety ceiling instead of local recovery" >&2
  exit 1
fi
