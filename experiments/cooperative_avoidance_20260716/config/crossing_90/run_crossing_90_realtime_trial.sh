...
CONFIG_DIR="$EXPERIMENT_DIR/config/crossing_90"
STEM="crossing_90_realtime_formal_trial_${TRIAL}"
...
rclpy.init()
node = rclpy.create_node(f"crossing_90_{stage.lower()}_clock_rate_check")
samples = []
...
(
  cd "$WORK_DIR"
  exec python3 run_dual_crossing_90.py
) >"$SIM_LOG" 2>&1 &
SIM_PID=$!
...
ros2 run epuck2_comm state_publisher --ros-args \
  ...
  -p mode:=periodic -p origin_x_m:=0.0 -p origin_y_m:=-0.35 \
  -p origin_yaw_rad:=1.5707963267948966 >"$STATE2_LOG" 2>&1 &
STATE2_PID=$!
...
stdbuf -oL -eL python3 "$CONFIG_DIR/run_crossing_90_controllers.py" \
  >"$CONTROLLER_LOG" 2>&1 &
CONTROLLER_PID=$!
