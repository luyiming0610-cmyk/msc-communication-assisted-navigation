...
CONFIG_DIR="$EXPERIMENT_DIR/config/combined_wood_moving_peer"
STEM="combined_local_then_peer_realtime_trial_${TRIAL}"
...
rclpy.init()
node = rclpy.create_node(f"combined_{stage.lower()}_clock_rate_check")
samples = []
...
stdbuf -oL -eL python3 "$CONFIG_DIR/run_combined_controllers.py" \
  >"$CONTROLLER_LOG" 2>&1 &
CONTROLLER_PID=$!
...
if (( task_complete_count < 1 || cooperative_complete_count < 1 )); then
  echo "Timed out waiting for cooperative recovery and box-pass completion" >&2
  exit 1
fi

local_enabled_count="$(grep -c 'local avoidance enabled=True, require sensors=True' "$CONTROLLER_LOG" 2>/dev/null || true)"
avoid_turn_count="$(grep -c 'mode=AVOID_TURN' "$CONTROLLER_LOG" 2>/dev/null || true)"
epuck1_local_count="$(grep -c 'epuck1.cooperative_avoider.*mode=LOCAL_' "$CONTROLLER_LOG" 2>/dev/null || true)"
epuck2_local_count="$(grep -c 'epuck2.cooperative_avoider.*mode=LOCAL_' "$CONTROLLER_LOG" 2>/dev/null || true)"
if (( local_enabled_count < 2 )); then
  echo "Both robots did not confirm fused local sensing" >&2
  exit 1
fi
if (( avoid_turn_count < 2 )); then
  echo "Both robots did not confirm peer-state CPA avoidance" >&2
  exit 1
fi
if (( epuck1_local_count < 1 )); then
  echo "epuck1 did not confirm local-obstacle priority takeover" >&2
  exit 1
fi
if (( epuck2_local_count != 0 )); then
  echo "epuck2 unexpectedly entered a local-obstacle mode" >&2
  exit 1
fi
first_local_line="$(grep -n -m1 'epuck1.cooperative_avoider.*mode=LOCAL_' "$CONTROLLER_LOG" | cut -d: -f1 || true)"
first_peer_line="$(grep -n -m1 'epuck1.cooperative_avoider.*mode=AVOID_TURN' "$CONTROLLER_LOG" | cut -d: -f1 || true)"
if [[ -z "$first_local_line" || -z "$first_peer_line" ]] || (( first_local_line >= first_peer_line )); then
  echo "Required local-obstacle-then-peer-event order was not observed" >&2
  exit 1
fi
if grep -q 'COMPLETE: maximum runtime reached' "$CONTROLLER_LOG"; then
  echo "Run ended by safety ceiling instead of recovery" >&2
  exit 1
fi
echo "EPUCK1_LOCAL_MODE_LOG_COUNT=$epuck1_local_count" | tee -a "$EXECUTION_LOG"
echo "EPUCK2_LOCAL_MODE_LOG_COUNT=$epuck2_local_count" | tee -a "$EXECUTION_LOG"
echo "EVENT_ORDER_PASS local_line=$first_local_line peer_line=$first_peer_line" | tee -a "$EXECUTION_LOG"
...
python3 -m epuck2_comm.analyze_cooperative_bag "$BAG_DIR" \
  >>"$EXECUTION_LOG" 2>&1
python3 "$CONFIG_DIR/analyze_combined_task.py" "$BAG_DIR" \
  >>"$EXECUTION_LOG" 2>&1
if [[ ! -s "$BAG_DIR/analysis/summary.json" ]]; then
  echo "Bag-derived summary is missing or empty" >&2
  exit 1
fi
if [[ ! -s "$BAG_DIR/analysis/combined_task_summary.json" ]]; then
  echo "Combined-task summary is missing or empty" >&2
  exit 1
fi
...
print(
    ...
    "control_input=fused_local_peer_and_wood"
)
PY

python3 - "$BAG_DIR/analysis/combined_task_summary.json" <<'PY' | tee -a "$EXECUTION_LOG"
import json
import sys

with open(sys.argv[1], encoding="utf-8") as stream:
    summary = json.load(stream)
if not summary["epuck1_passed_box"]:
    raise SystemExit("epuck1 did not pass the wooden box")
if any(summary["box_collision_detected"].values()):
    raise SystemExit("Robot-to-box collision check failed")
if summary["minimum_box_clearance_m"]["/epuck1/state"] < 0.005:
    raise SystemExit("epuck1 box clearance below 0.005 m acceptance gate")
print(
    "TASK_PASS "
    f"max_epuck1_x={summary['max_epuck1_x_m']:.6f} "
    f"epuck1_box_clearance={summary['minimum_box_clearance_m']['/epuck1/state']:.6f} "
    f"epuck2_box_clearance={summary['minimum_box_clearance_m']['/epuck2/state']:.6f}"
)
PY
